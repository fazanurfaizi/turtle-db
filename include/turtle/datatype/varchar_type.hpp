...
public:
  ~VarcharType() override = default;

  explicit VarcharType(DataType type);
  ...
  // VARCHAR doesn't support these operations
  ...
  auto is_inlined(const Value &val) const -> bool override;
};
