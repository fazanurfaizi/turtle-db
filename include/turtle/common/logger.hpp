#ifndef INCLUDE_COMMON_LOGGER_HPP_
#define INCLUDE_COMMON_LOGGER_HPP_
...
static constexpr auto pas_last_slash(cstr a, cstr b) -> cstr {
  return *a == '\0'  ? b
         : *a == '/' ? pas_last_slash(a + 1, a + 1)
                     : pas_last_slash(a + 1, b);
}

static constexpr auto pas_last_slash(cstr a) -> cstr {
  return pas_last_slash(a, a);
}

#define __SHORT_FILE__                                                         \
  ({                                                                           \
    constexpr cstr sf__{pas_last_slash(__FILE__)};                             \
    sf__;                                                                      \
  })
...
#define LOG_ERROR(...)                                                         \
  output_log_header(__SHORT_FILE__, __LINE__, __FUNCTION__, LOG_LEVEL_ERROR);  \
...
#define LOG_WARN(...)                                                          \
  output_log_header(__SHORT_FILE__, __LINE__, __FUNCTION__, LOG_LEVEL_WARN);   \
  ::fprintf(LOG_OUTPUT_STREAM, __VA_ARGS__);                                   \
  fprintf(LOG_OUTPUT_STREAM, "\n");                                            \
  ::fflush(stdout)
#else
...
#define LOG_INFO(...)                                                          \
  output_log_header(__SHORT_FILE__, __LINE__, __FUNCTION__, LOG_LEVEL_INFO);   \
  ::fprintf(LOG_OUTPUT_STREAM, __VA_ARGS__);                                   \
  fprintf(LOG_OUTPUT_STREAM, "\n");                                            \
  ::fflush(stdout)
#else
...
#define LOG_DEBUG(...)                                                         \
  output_log_header(__SHORT_FILE__, __LINE__, __FUNCTION__, LOG_LEVEL_DEBUG);  \
  ::fprintf(LOG_OUTPUT_STREAM, __VA_ARGS__);                                   \
  fprintf(LOG_OUTPUT_STREAM, "\n");                                            \
  ::fflush(stdout)
#else
...
#define LOG_TRACE(...)                                                         \
  output_log_header(__SHORT_FILE__, __LINE__, __FUNCTION__, LOG_LEVEL_TRACE);  \
  ::fprintf(LOG_OUTPUT_STREAM, __VA_ARGS__);                                   \
  fprintf(LOG_OUTPUT_STREAM, "\n");                                            \
  ::fflush(stdout)
#else
...
#endif // INCLUDE_COMMON_LOGGER_HPP_
