...
DeleteExecutor::DeleteExecutor(
    ExecutorContext *exec_ctx, const plans::DeletePlanNode *plan,
...
auto DeleteExecutor::next(std::vector<storage::table::Tuple> *tuple_batch,
  ...
  while (this->child_executor_->next(&child_batch, &child_rids, batch_size)) {
    for (size_t i = 0; i < child_rids.size(); ++i) {
      table_heap_->mark_delete(child_rids[i]);
      ++this->total_rows_deleted_;
    }
  }
